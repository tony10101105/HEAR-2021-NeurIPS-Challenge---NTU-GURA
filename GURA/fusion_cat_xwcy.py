...
class XWC_fusion(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.yamnet = yamnet_hear.Model(yamnet.inference.Model())
...
def load_model(model_file_path: str = "") -> torch.nn.Module:
    ...
    model.timestamp_embedding_size = 4096
    model.scene_embedding_size = 4096
...
def get_timestamp_embeddings(
    ...
    yamnet_embeddings = yamnet_hear.get_timestamp_embeddings(audio, self.yamnet)[0]
    yamnet_embeddings = tf.constant(yamnet_embeddings).numpy()
    yamnet_embeddings = torch.from_numpy(yamnet_embeddings).to(xlarge_embeddings.device)
    ...
    embeddings = merge(xlarge_embeddings, wav2vec2_embeddings, crepe_embeddings, yamnet_embeddings)
